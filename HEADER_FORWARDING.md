...
4. **Reconnection Handling**: If the MCP session drops, the `MCPConnectionManager` transparently reconnects and retries the tool call with headers intact

The `call_tool_with_reconnect` function in `mcpo/utils/main.py` handles the full lifecycle: it constructs a `CallToolRequest` with the `_meta` payload when headers are configured, passes it via `session.send_request()`, and automatically retries with the same `_meta` on `ClosedResourceError`.
